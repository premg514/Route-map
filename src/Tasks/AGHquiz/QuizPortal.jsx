...
const StatsDisplay = memo(({ statsData, easyTotal, mediumTotal, hardTotal, solvedCount }) => {
      ...
      <CircularProgress>
        <svg width="120" height="120" viewBox="0 0 120 120">
          ...
          <circle
            ...
            stroke="#d1fae5"
            strokeWidth="12"
            strokeDasharray={`${circumference / 3} ${(circumference * 2) / 3}`}
            strokeDashoffset="0"
          ...
          <circle
            ...
            stroke="#22c55e"
            strokeWidth="12"
            strokeDasharray={`${(easyPercentage / 100) * (circumference / 3)} ${circumference}`}
            strokeDashoffset="0"
            transform="rotate(-90 60 60)"
            strokeLinecap="round"
          />
          <circle
            ...
            stroke="#f59e0b"
            strokeWidth="12"
            strokeDasharray={`${(mediumPercentage / 100) * (circumference / 3)} ${circumference}`}
            strokeDashoffset={`${-circumference / 3}`}
            transform="rotate(-90 60 60)"
            strokeLinecap="round"
          />
          <circle
            ...
            stroke="#ef4444"
            strokeWidth="12"
            strokeDasharray={`${(hardPercentage / 100) * (circumference / 3)} ${circumference}`}
            strokeDashoffset={`${(-circumference * 2) / 3}`}
            transform="rotate(-90 60 60)"
            strokeLinecap="round"
          />
...
const DSADashboard = () => {
  ...
  const difficultyCounts = useMemo(() => {
    ...
    const easyProblems = levelFilteredProblems.filter(p => p?.difficulty === "Easy" && p?.solved).length;
    const mediumProblems = levelFilteredProblems.filter(p => p?.difficulty === "Medium" && p?.solved).length;
    const hardProblems = levelFilteredProblems.filter(p => p?.difficulty === "Hard" && p?.solved).length;
  ...
  const filterProblems = useCallback((problems, categoryTitle) => {
    if (!problems) return [];

    return problems.filter(problem => {
      // Filter by level (time duration)
      const levelMatch = problem.level && problem.level.includes(filters.activeLevel);

      // Filter by difficulty
      const difficultyMatch = filters.difficulty === "All" || problem.difficulty === filters.difficulty;

      // Filter by topic/category
      const topicMatch = filters.topic === "All" || categoryTitle === filters.topic;

      // Filter by search term
      const searchMatch = problem.name.toLowerCase().includes(filters.searchTerm.toLowerCase());
  ...
  const handleProblemUpdate = useCallback((categoryId, updatedProgress, updatedProblems) => {
  ...
  const filteredCategoryProblems = useMemo(() => {
    ...
    categories.forEach(categoryId => {
      if (problemsData[categoryId]) {
        ...
        const filtered = filterProblems(problemsData[categoryId], categoryTitle);
  ...
  return (
    <Container>
      <Header>
        ...
        <Subtitle>
          Sharpen your coding skills with handpicked problems and proven strategies.
        </Subtitle>
      </Header>
